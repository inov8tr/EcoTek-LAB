...
import { FlaskConical, Filter, Download, ArrowLeft } from "lucide-react";

function formatDate(date: Date) {
  return new Intl.DateTimeFormat("en-CA", { dateStyle: "medium" }).format(date);
}

type BinderTestsArchivedPageProps = {
  searchParams: Promise<{
    q?: string | string[];
  }>;
};

export default async function BinderTestsArchivedPage({ searchParams }: BinderTestsArchivedPageProps) {
  ...
  const where = {
    AND: [
      {
        status: BinderTestStatus.ARCHIVED,
      },
      q
        ? {
            name: {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">View binder tests that have been archived.</p>
        </div>
      ...
      <div className="rounded-xl border bg-card p-4">
        <div className="mb-4 flex flex-wrap items-center gap-3">
          <form className="flex flex-wrap items-center gap-3" method="get">
            ...
            {q && (
              <Link href={"/binder-tests/archived" as Route} className="text-sm text-muted-foreground hover:text-primary">
                Clear
              </Link>
            )}
          </form>
        ...
        {tests.length === 0 ? (
          <EmptyState
            ...
            title="No archived binder tests"
            description="Archived binder tests will appear here."
          />
        ) : (
