...
// Proxy/stub for Python parse; adjust PY_SERVICE_URL when available.
...
export async function POST(_req: Request, { params }: RouteParams) {
  ...
  if (!PY_SERVICE_URL || !API_KEY) {
    return NextResponse.json(
      { error: "Parse service not configured", status: 501 },
      { status: 501 },
    );
  }

  const res = await fetch(`${PY_SERVICE_URL}/binder-tests/${id}/parse`, {
    method: "POST",
    headers: {
      "x-api-key": API_KEY,
      "x-user-id": user.id,
  ...
  if (!res.ok) {
    return NextResponse.json(
      isJson ? JSON.parse(text) : { error: "Parse failed", status: res.status },
      { status: res.status },
    );
  }
